...
## Step 0 — 코드로 막을 수 있나  [Research]  ⭐ 먼저

문서에 적기 전에 **자동으로 걸리게 만들 수 있는지**부터 본다. 규칙은 읽어야 작동하고, 읽을지는 작업을 뭐라고 부르느냐에 달려 있어서 분류가 틀리는 지점에서 같이 무너진다(prism `AGENTS.md` 「규칙을 하나 더 쓰는 대신 게이트로 옮긴 이유」 · `plan-55` 판정: 규칙 58개 중 실제로 강제되는 것은 1개였다).

**Do:**
1. 이 함정이 걸릴 자리가 셋 중 어디인지 가른다.
   - **레포 안 파일이 바뀌는 순간** → pre-commit 게이트 후보. `.githooks/pre-commit`이 이미 도는 검사(`validate-design-system.sh` · `check-story-style.sh` · `check-unfounded-claims.sh` · drift 센서)에 조건 하나를 더하는 것으로 끝나는지 본다
   - **도구를 부르는 순간** (`use_figma` write · 새 파일 `Write` · `git push`) → Claude Code 훅(PreToolUse/PostToolUse) 후보. 선례: 44-5-2를 PostToolUse 훅으로 건 #418
   - **판단이 필요한 것** (「이게 결함인가」 「어느 쪽이 정본인가」) → 게이트로 못 막는다. 이때만 Step 1로 내려가 문서에 적는다
2. 게이트로 막을 수 있으면 **게이트를 먼저 만들고**, 문서에는 「어느 게이트가 막는다」 한 줄만 남긴다. 규칙 본문을 새로 쓰지 않는다.
3. 게이트로 못 막는 것만 아래 Step 1~4로 간다. 그 경우에도 규칙에 **트리거를 「목적」이 아니라 「지금 하고 있는 일」로** 적는다(`AGENTS.md` 색인 표의 원칙).

**Self-check:** 「다음 사람이 이 규칙을 안 읽어도 걸리는가?」에 아니오면, 정말 게이트로 못 만드는지 한 번 더 본다.
