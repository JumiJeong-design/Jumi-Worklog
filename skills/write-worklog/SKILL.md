...
**Do:**
1. 로컬 `~/Desktop/jumi-worklog/CONTEXT.md`를 읽는다. **실제 파일의 섹션 구조를 따른다** — 아래 매핑은 현재 구조 기준이고, 파일에 없는 섹션을 새로 만들지 않는다.
2. 오늘 worklog에서 다음을 추출해 해당 섹션을 **덮어쓴다**(이력 나열이 아니라 현재 상태 한 벌):
   - 오늘 완료한 주요 작업·기준 변화 → `## 현재 상태` 아래 해당 절(`### 운영 모드` / repo별 절) 갱신, 헤더의 기준 날짜도 오늘로
   - 새로 결정·종결된 사항 → `### 열려 있는 결정` 갱신. 종결 항목은 ~~취소선~~ + "무엇을 폐기했는지" 한 줄로 남긴다(다음 세션이 되돌리지 않도록)
   - 새로 발견한 함정 → `### 알려진 캐비엇`에 추가 (재발 가능한 것만)
   - 완료/신규 백로그 → `## 다음 후보 작업` 갱신
3. `Last updated: YYYY-MM-DD` 날짜를 오늘로 갱신한다. **이 줄에는 날짜만 쓴다** — 세션 요약을 괄호로 덧붙이지 않는다. 요약은 worklog가 정본이다. (2026-08-10 정정 — 이전 판이 존재하지 않는 섹션명을 지시해, 갈 곳 없는 요약이 이 줄에 6천 자까지 쌓였다. `check-context-freshness.sh`가 이 줄이 비대하면 경고한다)
