...
두 에이전트가 같은 레포(주로 `riiid/prism`)에서 동시에 일할 때, 충돌 없이 합쳐지도록 아래를 기본 규율로 한다. 시니어 엔지니어의 분업 원칙: **충돌은 사후 머지로 푸는 게 아니라 사전 경계로 방지한다.**

1. **소유 경계로 나눈다 (파일이 아니라 디렉터리/도메인 단위).**
   - 한 트랙이 소유한 디렉터리는 다른 트랙이 건드리지 않는다. 예: Claude Code = `.storybook/`·i18n 토큰, Codex = `src/markdown/`·해당 컴포넌트.
   - 같은 파일을 양쪽이 수정해야 하는 작업이면 병렬로 쪼개지 말고 **순차로** 잡는다.

2. **생성물은 단일 소유자.**
   - `pnpm token:build` 산출물(`theme.css`, `public.manifest.json` 등)은 **한 트랙만 커밋**한다. 다른 트랙은 직접 재생성·커밋하지 않고 rebase로 받아간다.
   - 토큰 정의(`*.tokens.json`)를 양쪽이 바꿔야 하면, 그 변경만 먼저 한 트랙이 머지하고 나머지가 따라간다.

3. **계약 우선(contract-first).**
   - 공유 인터페이스(토큰 이름, `component-contracts/*.md`, export 경로)는 **병렬 시작 전에** 합의·고정한다. 합의 전 구현 착수 금지.

4. **트랙별 브랜치 + rebase로 통합.**
   - 트랙마다 독립 브랜치. `main` 직접 공유 금지.
   - push 전 항상 `git pull --rebase`. merge 커밋이 아니라 rebase로 선형 유지.
   - 커밋은 작고 잦게 — 큰 덩어리 하나보다 합류 비용이 낮다.

5. **머지 순서를 정한다 (foundation first).**
   - 토큰/foundation 변경 → 컴포넌트 → 스토리 순으로 머지. 아래층(토큰)이 먼저 안정돼야 위층이 깨지지 않는다.
   - 한 트랙이 P0(선행조건)을 들고 있으면, 의존하는 트랙의 해당 작업은 P0 머지 후 시작한다.

6. **분담은 worklog plan에 명시한다.**
   - 그날 누가 어느 디렉터리를 소유하고 머지 순서가 어떤지 6/18처럼 plan에 적고 시작한다. 암묵적 분담은 금지.
