...
## 2026-06-27 추가 — 최근 운영 진화에서 뽑은 인사이트

읽은 범위: 2026-06-12 ~ 06-26 워크로그(`logs/2026/06`), `skills/`, `ops-plan.md`, 최근 커밋. 기존 12개 인사이트(AX 기본기)와 겹치지 않는, 운영방식이 진화하며 새로 생긴 후보다.

### 신규 인사이트 후보

#### 신규 1. 멀티 에이전트 격리 — 한 트리에 두 AI는 사고를 부른다

근거: 2026-06-25 "`git stash -u`가 Codex 활성 WIP를 쓸어담음 → `stash apply`로 복구", "공유 체크아웃에 다중 에이전트 동시 작업 = 고위험(plan HEAD가 세션 중 이동, 파일 불쑥 생성)", 2026-06-26 "규칙0 = 한 repo 한 에이전트 / 동시면 `git worktree` 격리", 좀비 watch 프로세스. 기존 인사이트는 단일 AI 전제였으나, Claude Code + Codex 동시 운영이 되며 충돌·격리·파일 소유권이 실제 운영 문제가 됐다.

#### 신규 2. AI 루프는 어디까지? — read-only 감사부터 자동화

근거: 2026-06-25 Figma↔코드 토큰 드리프트 감사 도구. "잡기 싼 층은 자동, 비싼 taste 층은 사람"에 부합하는 유일한 빈틈이 드리프트 감사였고, read-only라 승인 게이트와 안 부딪혀 거기만 루프화. 자동화 경계의 구체적 답 = 탐지는 자동, 결정은 사람.

#### 신규 3. 디자인 토큰 드리프트를 '이름'이 아니라 '값'으로 감사하기

근거: `scripts/figma-token-drift.mjs`. 이름 매칭은 거짓 드리프트(코드 `gray`/`neutral` 두 램프 + Figma 이름 규칙이 코드 경로와 1:1 아님) → "Figma 색이 코드 팔레트에 존재하는가"만 본다. 드리프트 2건 발견했지만 "어느 쪽으로 맞출지는 디자인 판단"이라 자동 처리 안 함.

#### 신규 4. 반복 운영 행동을 '스킬'로 박제한다

근거: `skills/` 8종(write-worklog·handoff-check·session-snapshot·record-trap·sync-entry·prep-meeting·bump-version). Compound 3편의 "취향을 token·rule·checklist로 남기기"의 다음 형태 = 반복 *행동*을 슬래시 커맨드 스킬로 남기기.

#### 신규 5. trap을 규칙으로 승격 — 같은 실수를 두 번 안 한다

근거: `record-trap` 스킬 + 2026-06-25 함정 다수 기록(stale 캐시·좀비 watch·외부 원복). taste→rule이 아니라 mistake→rule. 실수를 Prism 규칙 / 운영 규칙 / worklog 중 알맞은 곳으로 승격하는 루틴.

#### 신규 6. 측정 우선 — 스크린샷·캐시를 믿지 말고 실데이터를 읽어라

근거: 2026-06-25 `get_screenshot` stale 캐시(변경 직후 옛 화면) → 노드 fill 직접 읽어 실반영 확인, 좀비 `tsx --watch`가 "출력만" 갱신. AI 작업 검증의 운영 원칙.

#### 신규 7. 안 쓰기로 한 실험을 버리지 않고 '박제'한다

근거: GlassSurface 커서 sheen·WebGL Prism 배경 → 보류하되 롤백이 아니라 Explorations 스토리로 보존("기술 검증 끝나 학습 박제, 같은 탐색 반복 방지"). 보류한 탐색물의 보존 트랙.

#### 신규 8. 워크로그를 '읽는 도구'로 만든다 (문서 레벨 설계의 성숙형)

근거: `content/*.md` 정본 분리(모바일 직접 편집), 운영판 재설계(액션 카드 + 원칙/방향 접힘), 백로그 카드화·상태/필터, plan 임베드 정본화, 공개 worklog 뷰어. 기존 "문서가 많아지면 문서 레벨 설계가 필요하다"(인사이트 9)의 운영 시스템화.

### 기존 글 보강 포인트

- **Storybook 글 보강** — intrinsic(컴포넌트 내재 마이크로 인터랙션 = Storybook 최고 충실도) vs extrinsic(화면 간 여정·라우팅 = Figma/앱) 경계. 근거: 2026-06-25 "스토리북 상위 목적 재정의".
- **외부 컴포넌트 도입 판단** — "import 금지, 재구현". 우리가 이미 푼 문제(브라우저 호환 `backdrop-filter` 분기)를 되돌리지 않기. 근거: 2026-06-25 21st.dev 리퀴드 글래스 비교.
- **CI 분리** — 문서 게이트 실패가 시각 프리뷰를 막지 않게 게이트/프리뷰 job 분리 → 디자이너가 항상 프리뷰를 본다. 근거: PR #15 `release.yml` 3-job 분리.
