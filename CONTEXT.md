...
>
> 이 파일은 **누적 이력이 아니라 스냅샷**이다. 날짜별 경과는 `logs/`가 정본이므로 여기에 날짜 섹션을 쌓지 않고, 현재 상태 한 벌만 덮어쓴다(2026-07-14 정리 — 6/12~6/23 날짜 섹션 6개가 누적돼 worklog와 중복되고 갱신이 밀리던 것을 접음. 2026-08-10 정리 — `Last updated` 줄에 세션 요약이 괄호로 6천 자까지 쌓여 날짜만 남김. **이 줄에는 날짜만 쓴다** — 세션 요약은 worklog로, 상태 변화는 아래 본문 섹션 덮어쓰기로 간다).
...
- **홈/히스토리 §4.1·§4.9 정합은 08-11에 종결**(정본은 그날 worklog) — 빠르게 시작하기 전판(모바일·웹·DnD·태블릿·다크)=기획서 v1.0.6 키워드 칩 확정(웹 탭+문장 필 판은 참고용), 모드 토글=Deep On/Off 단일 명문으로 세그먼트 탐색 폐기, 채팅 목록 삭제·상태 7시트, 코치마크·칩 탭 Chat 전환(라이트/다크). 후속: 글로우 방향 결정 / 기획 요청 2건(칩 9종 변환 문장·유도 질문 데이터셋, 딥링크 매핑) / 코치마크 Quick·Deep 결과 2종 / prism `HistoryItem` loading 포팅.
