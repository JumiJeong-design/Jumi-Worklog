...
- **[미푸시] History 빈 목록 문구 통일 커밋 `a82e139`** — "새 대화를 시작해 보세요!"(기획서 §4.11) 피그마+코드 반영 완료, 커밋은 `feat/scroll-return-motion` 로컬 ahead 1. 병렬 세션이 같은 브랜치 작업 중이라 push는 브랜치 정리 때 함께. PlanLabel "Free"는 History/Footer 비로그인 variant 재작업 때 결정.
