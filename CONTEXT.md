...
- **딤은 30 단일이다(08-12 재확정).** `semantic/overlay/scrim` 라이트·다크 모두 `black-alpha/30`. 08-11에 2단(시트 30 / 모달 40)으로 나눴다가 **형제 노드 기준으로 다시 세니 "시트 50개"가 모달 32개를 포함한 오분류**여서 철회했고 `overlay/scrim-soft`는 삭제했다. 제품 딤 50개가 이 토큰에 묶여 있다. **코드는 아직 0.4/0.6이다**(`f23f86b`가 그 시점 피그마에 맞춘 것) — 0.3으로 내릴지 확인 필요. blur는 미채택(드래그 표면 재합성·Safari 중첩). 라이트박스 60% 8개는 컴포넌트 내부라 별건.
