...
- **FileCard 20px file-text 3건(1.8)을 첨부 칩(1.5)과 통일할지**(08-11) — stroke 판정에서 남은 유일한 결정(plan-10).
...
- **홈 placeholder·액센트 정본**(08-11) — 라이트=구 placeholder+액센트 / 다크=신 placeholder로 프레임 혼재. 확정돼야 홈 스토리 재정합 2차 착수 가능.
- **입력창 글로우 처분**(08-11) — 폐기 vs Deep On/포커스 전용 재활용.
- **6자리 코드 입력 컴포넌트**(08-11) — 신설 vs `Text Field` 조합. 이메일 플로우 시안 착수 전 결정.
- **`Popover`·`Menu` `className` 폭 API 모순** — `GlassSurface` 인라인 width 탓. 고치면 영향 범위가 넓다. / **`ProfileGroup` hover 미정의** — 피그마 축 추가 선행.
...
- **DS 게시 잔여** — `RetryButton` 세트만 두 번 연속 게시에서 빠져 CHANGED(위생분·비블로킹) — 다음 게시 때 다이얼로그 체크 확인. 게시된 `History Item` `State=loading`으로 제품 `history — 행 상태` 시안의 스피너 오버레이 교체(1분 거리).
...
- **로그인·설정 트랙(08-11 신규)** — ① 이메일 플로우 정식 시안(로그인·가입 3단계·6자리 코드·비밀번호 설정/찾기·중복 계정 팝업 3종, 보드 `6756:33130` 목업 기반) ② 설정 화면 §4.12 보완(문의·비밀번호 변경·회원 탈퇴 추가 + 스펙 외 처분 + 언어 선택 + 비로그인 판) ③ 탈퇴 플로우 시안(보드 `7552:154184` 대조).
