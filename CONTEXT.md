...
- **게이트 서브카피 기획 확정 대기(08-12)** — 구문구 `지금까지 나눈 대화는 그대로 이어집니다.`가 v1.0.6 §3.1(**기존 계정 로그인 시 비로그인 세션 즉시 삭제**)과 정면으로 어긋나 거짓이 됐다. 신규 가입에서만 참인데 모달은 두 경로가 갈리기 전에 뜬다. 지금 들어가 있는 `로그인하면 여러 AI에게 제한 없이 물어볼 수 있어요`는 **디자인 임시안**이고 KR·JP·EN 전부 미검토 초안이다(주미님: "일단 넣어놔 확정인척 쓰지만말고"). 미확정 표시 3곳 = 제품 파일 `📌 결정 노트 — 비로그인 게이트` 「주미님 확인」 · DS 변수 `auth/gate-sub` description · plan-26. **바꾸는 비용은 변수 하나** — 47곳이 한 변수를 본다. 주의: §4.9는 "삭제를 고지하지 말라"고 했지 "반대를 약속하라"고 하지 않았다.
