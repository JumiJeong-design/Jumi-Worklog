...
- **[PM 대기] 첨부 정책 1건으로 축소(08-09)** — ① 형식·용량 초과 = 에러 칩이 아니라 **즉시 차단+알림**(v1.0.6과 충돌, PM 확인 대기). ~~② 드롭 판정 범위~~는 08-09에 **앱 창 전체로 재확정**(8/6 채팅 영역 축소 철회, v1.0.6 원문과 일치 — 안건 해소). 같은 날 드롭존 톤 = 검정(action-primary) 확정(블루는 Deep 전용 강조색), 피그마 3컷·스토리 정합 + 흡수 비행 시나리오 컷 신설. PR #63 머지 대기.
