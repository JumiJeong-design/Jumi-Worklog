...
- **Callout 세트 2축 분리의 게시·수락(08-18)** — DS 세트를 `Type`{4톤} × `Layout`{inline,action}로 갈랐고 `Type=Type5` → `Type=info, Layout=action`이 됐다(이름만 변경 · 렌더 바이트 동일 · componentKey 유지). **라이브러리 게시와 제품 파일 업데이트 수락은 UI 작업이라 주미님 몫**이고 아직 안 됐다. 수락 전까지 제품 파일 SA-1a 3장은 구 이름을 보고한다(정상). 롤백은 변형 5개 이름 되돌리기.
