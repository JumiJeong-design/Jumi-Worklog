...
**P0 (다른 작업을 막고 있다)**
1. **`Thinking/Toggle` 패키지 포팅** — 사고과정이 전 화면에 깔리는데 코드가 0이다. **계약 파일을 먼저 못 만든다**(포팅 전 계약 불가 규칙) — 근거 문서 안 `## 패키지 포팅 계약 초안` 절이 출발점. 리포트·로딩 트랙 여럿이 여기 막혀 있다.
2. **브랜치 커밋 origin/main 미도달** — `f5ddebf`·`cf4428a`(Guidelines)·`f6281b7`(섹션↔버킷) 등이 `feat/edge-scrim-exploration`에만 있다. PR이 안 열리면 다음 세션이 낡은 main을 근거로 쓴다.
3. **`copied` 패키지 포팅** — 8/13에 피그마(`Copy` 축)만 끝냈다. 계약에 조건 명시됨(눌린 표면 · 자동 복귀는 컴포넌트 소유 · `aria-live` · **토스트를 대체하지 않음**).

**P1 (스펙 위반·화면 오류)**
4. 웹 260 상태 프레임 3장 신규(로딩·실패·비로그인) — 상태 시안이 393 모바일에만 있어 웹 퍼스트와 어긋난다
5. `생성 중 행 hover 툴팁`을 웹 260으로 — PC 전용(§4.9)인데 모바일에 그려져 있다
6. `Toast` 닫기 32 → 44 (9건 · DS 픽셀 변경이라 **승인 필요**)
7. `AttachmentChip` 처리 중을 결정형 진행으로 (룰 39 · **피그마 선행**)
...
**공통 교훈: 이월 메모의 전제부터 실측한다.** 8/13 판단 9건 중 절반 이상이 "고칠 게 아니었다".
...
- **DS 변경은 기본값 = 현행 렌더로 설계한다.** 8/13 신설 7종이 전부 픽셀 변화 0이라 게시가 안전했다.
