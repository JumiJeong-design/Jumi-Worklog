...
- **룰 39 후속 미결 4건(08-13)** — ① 로딩 문구 `목록을 불러오는 중입니다.`를 되살릴지(스켈레톤 전환으로 소멸 · **기획서 규정 여부 미확인** · 규정돼 있으면 룰 35로 복원) ② `Empty State`의 `no-results`·`error` 슬롯도 `media` 불리언으로 통일할지(지금은 `no-chat`만 불리언이라 `Type=error` 쓰는 화면마다 손으로 꺼야 한다) ③ `Skeleton` 세트/컴포넌트 처분(소비처 0 + 계약 제외 · 폐기 vs 오토레이아웃 수리 · 소비처 0인 지금이 제일 싸다) ④ `AttachmentFile` 진행률을 `meta` 텍스트로 줄지. **일러스트 슬롯은 주미님 확정 = 필요**(불리언으로 되살림).
...
- **[룰 39 코드 후속]** 피그마는 끝났고 코드가 남았다. ① `ChatHistoryPanel`에 `status` 축(`idle|loading|error|empty`) — 지금 코드엔 loading·error 개념 자체가 없다 ② `Button` `state="loading"`이 `...` 텍스트다(`Button.tsx:98`) — 피그마 `State=loading` 확인 후 맞추는 단순 동기화, 게시 무관 ③ `AttachmentChip` 처리 중을 결정형 진행으로(업로드는 바이트를 안다). 피그마 잔여 = **웹 260 상태 프레임 3장**(로딩·실패·비로그인)과 `생성 중 행 hover 툴팁` 프레임을 웹으로 이동(PC 전용인데 393 모바일에 그려져 있다).
